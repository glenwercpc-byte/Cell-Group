...
const KEEPER_MENUS=['monthly','monthlyAll','yearly'];
const KEEPER_PW='1424';

function doExport(t){
  closeExport();
  if(KEEPER_MENUS.includes(t)){
    openKeeperAuth(t);
  } else if(t==='gdocs'){
    exportToGoogleDocs();
  } else if(t==='addressBook'){
    openAddressBook();
  }
}

function openKeeperAuth(target){
  const labels={'monthly':'월 샘터보고서','monthlyAll':'월 전체 보고서','yearly':'샘터별 출석 상황'};
  openFullModal(
    '<div style="background:#fff;border-radius:12px;width:100%;max-width:320px;padding:28px 24px 24px;position:relative;margin:auto">'
    +'<button onclick="closeFullModal()" style="position:absolute;top:14px;right:16px;background:#f0f0f0;border:none;border-radius:50%;width:28px;height:28px;font-size:.8rem;cursor:pointer">✕</button>'
    ...
    +'<div style="display:flex;gap:8px">'
    +'<button onclick="closeFullModal()" style="flex:1;padding:10px;border-radius:7px;font-size:.82rem;font-weight:600;background:#f0f0f0;color:#555;border:none;cursor:pointer">취소</button>'
    +'<button onclick="confirmKeeperAuth()" style="flex:1;padding:10px;border-radius:7px;font-size:.82rem;font-weight:600;background:#1a2744;color:#fff;border:none;cursor:pointer">확인</button>'
    +'</div>'
    +'</div>'
  );
  setTimeout(()=>{
    const inp=document.getElementById('keeper-pw');
    if(inp){
      inp.focus();
      inp.addEventListener('keydown',e=>{
        if(e.key==='Enter') confirmKeeperAuth();
...
function confirmKeeperAuth(){
  const inp=document.getElementById('keeper-pw');
  const target=inp?.dataset.target||'';
  const pw=(inp?.value||'').trim();
  const err=document.getElementById('keeper-pw-err');
  if(pw!==KEEPER_PW){
    if(err) err.textContent='비밀번호가 틀렸습니다.';
    if(inp){inp.value='';inp.focus();}
    return;
  ...
  closeFullModal();
  if(target==='monthly')     openMonthlyModal();
  if(target==='monthlyAll')  openMonthlyAllModal();
  if(target==='yearly')      openYearlyModal();
}
...
function exportToGoogleDocs(){
  ...
  const html='<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>시카고 언약 장로교회 '+currentYear+'년 샘터 조직표</title>'
    ...
    +'<button class="btn" id="gsheet-btn" style="background:#0f9d58">📊 구글시트 출력</button>'
  ...
  // 구글시트 출력 버튼 — 메인 창에서 직접 처리 (window.opener 의존 제거)
  setTimeout(()=>{
    const btn=w.document.getElementById('gsheet-btn');
    if(btn){
...
async function checkMonthlyExists(){
  ...
  if(exists){
    ...
    body.innerHTML=
      ...
      +'<div style="font-size:.9rem;color:#444;margin-bottom:20px;line-height:1.6">'+'<strong>'+mon+'월 보고서를 작성하시겠습니까?</strong></div>'
      +'<div style="display:flex;gap:10px;justify-content:center">'
      +'<button onclick="declineMonthlyCreate()" style="padding:9px 22px;background:#f0f0f0;color:#555;border:none;border-radius:7px;font-size:.85rem;cursor:pointer">아니요</button>'
      +'<button onclick="renderMonthlyForm()" style="padding:9px 22px;background:#1a2744;color:#fff;border:none;border-radius:7px;font-size:.85rem;font-weight:600;cursor:pointer">네</button>'
      +'</div></div>';
  }
}

function declineMonthlyCreate(){
  const body=document.getElementById('monthly-form-body');
  if(body) body.innerHTML='<p style="color:#888;padding:30px;text-align:center">샘터와 월을 선택하세요.</p>';
}
...
async function loadMonthThenRenderAll(){
  ...
  const toLoad=allSamters.filter(sNum=>
    !(attData[currentYear][sNum] && attData[currentYear][sNum][String(mon)]!==undefined)
  ...
  for(let i=0;i<toLoad.length;i+=BATCH){
    ...
    await Promise.all(batch.map(async sNum=>{
      try{
        ...
        attData[currentYear][sNum][String(mon)] = (res?.data !== null && res?.data !== undefined) ? res.data : null;
      }catch(e){
        if(!attData[currentYear][sNum]) attData[currentYear][sNum]={};
        attData[currentYear][sNum][String(mon)]=null; // 오류도 미제출로 처리
      }
      updateProgress();
    }));
  }
...
function renderMonthlyAll(){
  ...
  state.forEach(dist => {
    ...
    dist.samters.forEach(s => {
      ...
      const monthRecord = attData[currentYear]?.[s.num]?.[String(mon)];
