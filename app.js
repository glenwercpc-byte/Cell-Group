...
function exportToGoogleDocs() {
  ...
  state.forEach((dist, di) => {
    ...
    dist.samters.forEach(s => {
      ...
      for (let r = 0; r < Math.max(members.length, 1); r += 10) {
        ...
        rows += '<tr>';
        if (r === 0) {
          rows += '<td rowspan="' + rCount + '" style="border:1px solid #bbb;padding:3px 5px;text-align:center;font-weight:700;font-size:12px;background:#e8eef7;vertical-align:middle;white-space:nowrap">' + s.num + '</td>';
          rows += '<td rowspan="' + rCount + '" style="border:1px solid #bbb;padding:3px 6px;text-align:center;font-size:11px;background:#f2f5fa;vertical-align:middle;white-space:nowrap">' + s.keeper + '<br><span style="color:#3a5a8c;font-size:10px">(' + cnt + '명)</span></td>';
        }
        // 조원 10칸 — 동일 너비 (전체의 7%씩)
        chunk.forEach(n => {
          rows += '<td style="border:1px solid #ddd;padding:3px 4px;font-size:11px;white-space:nowrap;width:7%">' + (n || '&nbsp;') + '</td>';
        });
        rows += '</tr>';
      }
  ...
  const html = '<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8">'
    ...
    + 'body{font-family:"Noto Sans KR","Malgun Gothic",sans-serif;padding:12px 16px;font-size:11px;color:#111}'
    + 'h1{font-family:"Nanum Myeongjo",serif;font-size:16px;color:#1a2744;text-align:center;font-weight:800;margin-bottom:2px}'
    + '.sub{text-align:center;font-size:11px;color:#666;margin-bottom:8px}'
    + 'table{border-collapse:collapse;width:100%;table-layout:fixed}'
    + 'td{word-break:keep-all}'
    + '.guide{background:#fff8e8;border:1px solid #f0d080;border-radius:4px;padding:7px 10px;font-size:11px;color:#6b4c00;margin-bottom:8px;line-height:1.5}'
    + '.btn{display:inline-block;margin:0 5px 0 0;padding:5px 12px;background:#1a2744;color:#fff;border:none;border-radius:4px;font-size:11px;cursor:pointer;font-family:inherit;text-decoration:none}'
    + '.btn-g{background:#0f9d58}'
    + '@media print{.no-print{display:none!important}body{padding:5px 8px}@page{size:A4 landscape;margin:8mm}}'
    + '</style></head><body>'
