...
function apiCall(data) {
  return new Promise((resolve,reject) => {
    ...
    s.id=cb; s.src=API_URL+'?callback='+cb+'&payload='+encodeURIComponent(JSON.stringify(data));
    s.onerror=()=>{clearTimeout(timer);delete window[cb];s.remove();reject(new Error('네트워크 오류'));};
    document.head.appendChild(s);
  });
}

function cvtOrg(districts) {
  const r=[];
  Object.entries(districts).forEach(([n,d])=>{
    const sl=(d.samters||[]).map(s=>({num:s.num,keeper:s.keeper||'',members:(s.members||[]).filter(Boolean)}));
    if(sl.length>0) r.push({name:n,samters:sl});
  });
...
function saveCurrentToAllData() {
  allData[currentYear]=state.map(d=>({name:d.name,samters:d.samters.map(s=>({num:s.num,keeper:s.keeper,members:s.rows.flat().filter(Boolean)}))}));
}
...
function toggleYP(){
  const p=document.getElementById('yp'),opening=p.classList.contains('hidden');
  ...
  if(opening){
    ...
    const se=document.getElementById('yp-saved'),le=document.getElementById('yp-saved-list');
    if(!others.length){se.classList.add('hidden');}
    else{se.classList.remove('hidden');le.innerHTML='';others.forEach(y=>{const d=document.createElement('div');d.className='yp-saved-item';d.innerHTML='<span class="yn">'+y+'년</span><span class="yb-saved">저장됨</span>';d.onclick=()=>selectYear(String(y));le.appendChild(d);});}
    document.getElementById('new-year-inp').value='';document.getElementById('yp-err').textContent='';
...
function submitNewYear(){
  const val=document.getElementById('new-year-inp').value.trim(),y=parseInt(val),err=document.getElementById('yp-err');
  ...
  err.textContent='';selectYear(String(y));
}
function selectYear(y){
  closeYP();if(y===currentYear)return;saveCurrentToAllData();
  if(allData[y]){loadYear(y);return;}
  pendingYear=y;selMode=null;
  document.getElementById('modal-area').innerHTML=`<div class="modal-bd"><div class="modal-box"><div class="modal-title">${y}년 조직표 만들기</div><div class="modal-sub">${currentYear}년 조직표를 기반으로 ${y}년을 시작합니다.</div><div class="modal-opts"><button class="modal-opt" id="opt-copy" onclick="pickMode('copy')"><div class="opt-t">1. 현재 인원 그대로 사용</div><div class="opt-d">${currentYear}년 명단을 ${y}년으로 복사합니다.</div></button><button class="modal-opt" id="opt-fresh" onclick="pickMode('fresh')"><div class="opt-t">2. 전면 재조정</div><div class="opt-d">구조만 유지하고 조원 명단을 비웁니다.</div></button></div><div class="modal-btns"><button class="mb-cancel" onclick="cancelModal()">취소</button><button class="mb-ok" id="mok" onclick="confirmModal()" disabled>확인</button></div></div></div>`;
}
...
function confirmModal(){
  ...
  allData[y]=selMode==='copy'?JSON.parse(JSON.stringify(prev)):prev.map(d=>({name:d.name,samters:d.samters.map(s=>({num:s.num,keeper:'',members:[]}))}));
  document.getElementById('modal-area').innerHTML='';pendingYear=null;selMode=null;
  loadYear(y);toast(y+'년 조직표를 시작합니다','ok');
}
...
async function syncAllToSheets(){
  const targets=[];
  state.forEach(dist=>dist.samters.forEach(s=>{if(dirtySet.size===0||dirtySet.has(String(s.num)))targets.push({dist,samter:s});}));
  if(!targets.length){toast('변경 내용 없음','ok');return;}
  toast('저장 중… (0/'+targets.length+')','ok');
  let done=0,fail=0;
  for(const {dist,samter} of targets){
    try{
      await apiCall({action:'saveSamter',year:currentYear,samter:samter.num,district:dist.name,keeper:samter.keeper,members:samter.rows.flat().filter(Boolean)});
      done++;toast('저장 중… ('+done+'/'+targets.length+')','ok');
...
function doAddSamter(){
  const val=document.getElementById('si').value.trim(),err=document.getElementById('dp-err');
  if(!val||isNaN(Number(val))){err.textContent='숫자를 입력하세요 (예: 14)';return;}
  const sNum=val,dNum=parseInt(val[0]),dist=state.find(d=>d.name===dNum+'지구');
...
function render(){
  ...
  state.forEach((dist,di)=>{
    ...
    const ni=document.createElement('input');ni.value=dist.name;ni.style.cssText='background:transparent;border:none;color:#fff;font-weight:700;font-size:.82rem;padding:0;font-family:inherit;width:46px;text-align:left;outline:none';
    ni.addEventListener('input',()=>{dist.name=ni.value;});tt.appendChild(ni);
    ...
    dist.samters.forEach((samter,si)=>{
      ...
      samter.rows.forEach((row,ri)=>{
        ...
        row.forEach((v,ci)=>{
          ...
          const inp=document.createElement('input');inp.value=v;inp.placeholder=(ri*10+ci+1)+'번';
          ...
          inp.addEventListener('keydown',e=>{
            ...
            else{const nri=ri+1;if(nri>=samter.rows.length){samter.rows.push(['','','','','','','','','','']);render();setTimeout(()=>document.getElementById('g-'+samter.id+'-'+nri)?.querySelectorAll('input')[0]?.focus(),20);}else{document.getElementById('g-'+samter.id+'-'+nri)?.querySelectorAll('input')[0]?.focus();}}
          });
...
function updateStat(){
  const d=state.length,s=state.reduce((a,ds)=>a+ds.samters.length,0);
  const m=state.reduce((a,ds)=>a+ds.samters.reduce((b,sm)=>b+sm.rows.flat().filter(Boolean).length+(sm.keeper?1:0),0),0);
  const el=document.getElementById('stat');if(el)el.textContent=currentYear+'년 · '+d+'지구 · '+s+'샘터 · 총 '+m+'명';
}
...
function exportToGoogleDocs(){
  ...
  state.forEach((dist,di)=>{
    const chief=dist.samters[0]?.keeper||'-';
    rows+='<tr>'
      +'<td style="background:#3a5a8c;color:#fff;font-weight:700;font-size:12px;padding:5px 6px;border:1px solid #2a4a7c;white-space:nowrap;text-align:center">샘터</td>'
      +'<td style="background:#3a5a8c;color:#fff;font-weight:700;font-size:12px;padding:5px 6px;border:1px solid #2a4a7c;white-space:nowrap;text-align:center">청지기</td>'
      +'<td colspan="10" style="background:#3a5a8c;color:#fff;font-weight:700;font-size:12px;padding:5px 10px;border:1px solid #2a4a7c;white-space:nowrap">'+dist.name+'&nbsp;&nbsp;(지구장: '+chief+')</td>'
      +'</tr>';
  ...
  const html='<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>시카고 언약 장로교회 '+currentYear+'년 샘터 조직표</title>'
    +'<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:"Noto Sans KR","Malgun Gothic",sans-serif;padding:12px 16px;font-size:11px;color:#111}h1{font-family:"Nanum Myeongjo",serif;font-size:16px;color:#1a2744;text-align:center;font-weight:800;margin-bottom:2px}.sub{text-align:center;font-size:11px;color:#666;margin-bottom:8px}table{border-collapse:collapse;width:100%;table-layout:fixed}td{word-break:keep-all}.guide{background:#fff8e8;border:1px solid #f0d080;border-radius:4px;padding:7px 10px;font-size:11px;color:#6b4c00;margin-bottom:8px;line-height:1.5}.btn{display:inline-block;margin:0 5px 0 0;padding:5px 12px;background:#1a2744;color:#fff;border:none;border-radius:4px;font-size:11px;cursor:pointer;font-family:inherit;text-decoration:none}.btn-g{background:#0f9d58}@media print{.no-print{display:none!important}body{padding:5px 8px}@page{size:A4 landscape;margin:8mm}}</style>'
    +'</head><body>'
    +'<h1>시카고 언약 장로교회 '+currentYear+'년 샘터 조직표</h1>'
    +'<p class="sub">'+state.length+'지구 &middot; '+totalS+'샘터 &middot; 총 '+total+'명</p>'
    +'<div class="guide no-print"><strong>Google Docs 붙여넣기:</strong> 아래 "표 전체 선택" &rarr; 복사(Ctrl+C) &rarr; <a href="https://docs.google.com/document/create" target="_blank">Google Docs 새 문서</a> &rarr; 붙여넣기(Ctrl+V)</div>'
    +'<div class="no-print" style="margin-bottom:8px"><button class="btn" onclick="sel()">📋 표 전체 선택</button><a class="btn btn-g" href="https://docs.google.com/document/create" target="_blank">🔗 Google Docs 열기</a><button class="btn" onclick="window.print()">🖨 인쇄 (A4 가로)</button></div>'
...
async function saveMonthlyData(){
  ...
  if(!attData[currentYear])attData[currentYear]={};if(!attData[currentYear][sNum])attData[currentYear][sNum]={};
  attData[currentYear][sNum][mon]=rec;try{localStorage.setItem(ATT_KEY,JSON.stringify(attData));}catch(e){}
...
function renderYearlyTable(){
  const sNum=document.getElementById('yr-samter')?.value,body=document.getElementById('yearly-table-body');
  if(!sNum||!body)return;
  ...
  members.forEach((name,idx)=>{
    let attM=0,cells='';
    for(let m=1;m<=12;m++){const rec=attData[currentYear]?.[sNum]?.[m];if(!rec){cells+='<td style="border:1px solid #ddd;text-align:center;color:#ccc">-</td>';continue;}const v=rec[name]||'';if(v==='O')attM++;const col=v==='O'?'#2d6a4f':v==='X'?'#c0392b':'#888';cells+='<td style="border:1px solid #ddd;text-align:center;color:'+col+';font-weight:'+(v?'700':'400')+'">'+( v||'·')+'</td>';}
